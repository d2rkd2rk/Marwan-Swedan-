...
export async function POST(request:Request){
  try{
    ...
    if(!ok){await securityEvent(user.id,'failed_login','medium',request,{identifier:value});return NextResponse.json({error:'Invalid email/username or password.'},{status:401})}
