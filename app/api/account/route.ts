...
import {requireUser,hashPassword,validPassword} from '@/lib/auth';
import {audit} from '@/lib/security';

export async function GET(){
  try{return NextResponse.json({user:await requireUser()})}catch(e:any){return NextResponse.json({error:e.message==='FORBIDDEN'?'Forbidden':'Authentication required'},{status:401})}
}
...
export async function PATCH(request:Request){
  try{
    ...
    const rows=await db`update users set name=${name},username=${username},bio=${bio} where id=${user.id} returning id,name,email,whatsapp,username,role,is_blocked,blocked_until,bio,avatar_url,cv_url,cv_name`;
    await audit(user.id,'profile_update',request);
    return NextResponse.json({user:rows[0]});
  }catch(e:any){return NextResponse.json({error:e.message==='FORBIDDEN'?'Forbidden':'Authentication required'},{status:e.message==='UNAUTHENTICATED'?401:400})}
}
