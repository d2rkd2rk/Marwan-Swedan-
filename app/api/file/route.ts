...
export async function GET(request:NextRequest){
  try{
    ...
    return new NextResponse(result.stream,{headers:{'Content-Type':result.blob.contentType,'X-Content-Type-Options':'nosniff','Cache-Control':'private, no-cache',ETag:result.blob.etag}});
  }catch{return new NextResponse('File unavailable',{status:404})}
}