...
export async function POST(request:Request){try{const admin=await requireAdmin();await ensureMediaColumns();const b=await request.json();const courseId=String(b.courseId||'');const title=String(b.title||'').trim();if(!courseId||!title)return NextResponse.json({error:'Course and lesson title are required.'},{status:400});const fileUrl=b.fileUrl?String(b.fileUrl):null;validateFileUrl(fileUrl);const slug=await uniqueSlug(courseId,String(b.slug||title));const rows=await db`insert into lessons(course_id,title,slug,position,duration_minutes,video_url,file_url,file_name,content,published) values(${courseId},${title},${slug},${Number(b.position||0)},${Math.max(0,Number(b.durationMinutes||0))},${b.videoUrl||null},${fileUrl},${b.fileName||null},${b.content||''},${Boolean(b.published)}) returning *`;await audit(admin.id,'create_lesson',request,{lessonId:rows[0].id});return NextResponse.json({lesson:rows[0]},{status:201})}catch(e:any){console.error('CREATE_LESSON_FAILED',e);const message=e.message==='INVALID_FILE_URL'||e.message==='INVALID_FILE_PATH'?'Invalid course file reference.':e.message==='FORBIDDEN'?'Forbidden':'Could not create lesson';return NextResponse.json({error:message},{status:e.message==='FORBIDDEN'?403:400})}}

export async function PUT(request:Request){try{const admin=await requireAdmin();await ensureMediaColumns();const b=await request.json();const id=String(b.id||'');const courseId=String(b.courseId||'');const title=String(b.title||'').trim();if(!id||!courseId||!title)return NextResponse.json({error:'Lesson id, course and title are required.'},{status:400});const fileUrl=b.fileUrl?String(b.fileUrl):null;validateFileUrl(fileUrl);const slug=await uniqueSlug(courseId,String(b.slug||title),id);const rows=await db`update lessons set title=${title},slug=${slug},position=${Number(b.position||0)},duration_minutes=${Math.max(0,Number(b.durationMinutes||0))},video_url=${b.videoUrl||null},file_url=${fileUrl},file_name=${b.fileName||null},content=${b.content||''},published=${Boolean(b.published)} where id=${id} and course_id=${courseId} returning *`;if(!rows.length)return NextResponse.json({error:'Lesson not found'},{status:404});await audit(admin.id,'update_lesson',request,{lessonId:id});return NextResponse.json({lesson:rows[0]})}catch(e:any){console.error('UPDATE_LESSON_FAILED',e);const message=e.message==='INVALID_FILE_URL'||e.message==='INVALID_FILE_PATH'?'Invalid course file reference.':e.message==='FORBIDDEN'?'Forbidden':'Could not update lesson';return NextResponse.json({error:message},{status:e.message==='FORBIDDEN'?403:400})}}
