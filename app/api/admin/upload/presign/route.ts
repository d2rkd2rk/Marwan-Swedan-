...
const allowed=new Set(['video/mp4','video/webm','video/quicktime','audio/mpeg','audio/mp4','application/pdf','application/zip','application/x-zip-compressed','text/plain','text/markdown','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.ms-powerpoint','application/vnd.openxmlformats-officedocument.presentationml.presentation']);
export async function POST(request:Request){
 try{
  ...
  return NextResponse.json({uploadUrl:presignedUrl,url:`/api/file?pathname=${encodeURIComponent(pathname)}`,pathname,name});
