...
export function AuthForm({mode}: {mode: 'login' | 'register'}) {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (mode === 'login' && json.requiresOtp) {
  ...
  const verify = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      location.href = json.user?.role === 'admin' ? '/admin' : '/courses';
    } catch {
      setError('Unable to connect to the server.');
    } finally {
      setBusy(false);
    ...
    <div className="form">
      ...
      {challengeId ? (
        <>
          ...
          <p className="muted">We sent a 6-digit verification code to your account email. It expires in 10 minutes.</p>
            ...
            {mode === 'register' && (
              <>
                <div className="field">
                  ...
                  <input required minLength={2} maxLength={80} name="name" />
                </div>
                <div className="field">
                  <label>Email</label>
            ...
            {mode === 'login' && (
              <>
                <div className="field">
                  ...
                  <input required name="identifier" autoComplete="username" />
                </div>
                <div className="field">
                  <label>Password</label>
                  <div style={{display: 'flex', gap: 8}}>
