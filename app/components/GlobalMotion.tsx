...
export default function GlobalMotion(){
 useEffect(()=>{
  ...
  const click=(e:MouseEvent)=>{const b=document.createElement('div');b.className='clickBurst';b.style.left=`${e.clientX}px`;b.style.top=`${e.clientY}px`;document.body.appendChild(b);b.addEventListener('animationend',()=>b.remove(),{once:true});const target=e.target as HTMLElement;if(target&&target.closest){const interactive=target.closest('button,a,.btn');if(interactive)gsap.fromTo(interactive,{scale:1},{scale:.97,duration:.08,yoyo:true,repeat:1,ease:'power2.out',overwrite:true})}};
